...
assert.match(openGame,/if\(state\.globalTreeSideFilter!==nextScope\)/);
assert.match(openGame,/state\.globalTree=null;state\.globalTreeBuiltFor=0/);
assert.match(openGame,/state\.globalTreePendingFilter=null/);
assert.equal((openGame.match(/state\.globalTree=null/g)||[]).length,1,"openGame ne doit invalider l'arbre qu'à l'intérieur du garde de filtre");

console.log("CURRENT-VERSION OPEN-GAME GLOBAL-TREE REGRESSION TEST OK");
