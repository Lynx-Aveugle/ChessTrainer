...
assert.equal((app.match(/trainingController\.scan\(\)/g)||[]).length,1);assert.ok((app.match(/trainingController\.stop\(\)/g)||[]).length>=1);assert.equal((app.match(/trainingController\.exportFile\(\)/g)||[]).length,1);assert.equal((app.match(/trainingController\.clear\(\)/g)||[]).length,1);
